...
const getStartOfDayUtc = (date) => new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()));
...
const updateDailyStreaks = async () => {
    ...
    for (const userActivity of activeUsers) {
        const user = await User.findById(userActivity._id).select("currentStreak highestStreak lastStreakDate");
        if (!user) {
            continue;
        }
        ...
        const shouldIncrement = lastStreakDate && lastStreakDate.getTime() === yesterdayStart.getTime();
...
export const startStreakCron = () => {
    cron.schedule("5 0 * * *", () => {
        updateDailyStreaks().catch((error) => {
